...
# 【必填】模型文件路径
# --------------------------------------------------
MODEL="/storage/emulated/0/Download/AI模型/Qwen2.5-Coder-7B-Instruct-Q4_K_M.gguf"
...
# 【核心参数】上下文与生成控制
# --------------------------------------------------
CTX_SIZE=8192                # 上下文长度（令牌数），模型原生支持32K。调大能让模型参考更多前文，但会增加显存；调小则节省资源。
N_PREDICT=512                # 最大生成令牌数，社区推荐值。调大回复更详细，但可能偏离主题；调小回复更简洁。
TEMP=0.7                     # 温度，官方及社区推荐值。调大输出更多样，但可能产生幻觉；调小输出更确定、更保守。
...
TOP_P=0.9                    # 核采样（Top-P），社区推荐值。调大允许更多词被考虑；调小只保留最核心的词。
MIN_P=0.05                   # 最小概率阈值，过滤掉概率低于此值的词。调大过滤更多；调小保留更多稀有词。
REPEAT_PENALTY=1.05          # 重复惩罚系数。调大更严厉避免重复；调小则允许更多重复。
