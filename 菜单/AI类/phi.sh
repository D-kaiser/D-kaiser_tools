...
# 【必填】模型文件路径
# --------------------------------------------------
MODEL="/storage/emulated/0/Download/AI模型/Phi-4-mini-Instruct-Q4_K_M.gguf"
...
# 【核心参数】上下文与生成控制
# --------------------------------------------------
CTX_SIZE=8192                 # 上下文长度（令牌数），模型原生支持128K[reference:8][reference:9]。调大能让模型参考更多前文，但会显著增加显存；调小则节省资源。
N_PREDICT=512                 # 最大生成令牌数，社区推荐值[reference:10]。调大回复更详细，但可能偏离主题；调小回复更简洁。
TEMP=0.8                      # 温度，微软官方推荐值[reference:11][reference:12]。调大输出更多样，但可能产生幻觉；调小输出更确定、更保守。
...
# 【高级参数】采样与重复惩罚
# --------------------------------------------------
TOP_K=40                      # Top-K 采样，限制候选词为概率最高的K个。调大增加候选多样性；调小则更集中。
TOP_P=0.95                    # 核采样（Top-P），微软官方推荐值[reference:13][reference:14]。调大允许更多词被考虑；调小只保留最核心的词。
MIN_P=0.05                    # 最小概率阈值，过滤掉概率低于此值的词。调大过滤更多；调小保留更多稀有词。
REPEAT_PENALTY=1.0            # 重复惩罚系数，社区推荐值[reference:15]。调大更严厉避免重复；调小则允许更多重复。
REPEAT_LAST_N=64              # 重复惩罚考虑的上文长度。调大考虑更长的历史；调小只关注最近内容。
